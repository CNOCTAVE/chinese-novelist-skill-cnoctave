...
## 创作步骤（对每一章依次执行）
1. **首先读取 {projectPath}/01-大纲.json**，找到当前章节的规划信息（核心事件、承接上章、悬念钩子、出场人物、场景列表）
...
8. 深度润色（去除AI味）
9. 再次字数检查
10. 在 01-大纲.json 追加 300-500 字章节摘要
...
3. **首先读取 {projectPath}/01-大纲.json**，找到该章节的规划信息（核心事件、承接上章、悬念钩子、出场人物、场景列表）
