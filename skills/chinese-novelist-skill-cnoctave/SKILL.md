...
> **路径约定**：本文档中所有 `{CWD}/` 开头的路径均指**用户工作目录**。`{CWD}` 即用户启动写作会话时的当前目录（非 skill 所在目录），所有生成的文件/目录都应创建在此路径下，每个小说项目一个独立子文件夹（`{CWD}/{timestamp}-{小说名称}/`）。
...
创建项目文件夹（`{CWD}/{timestamp}-{小说名称}/`），生成大纲、人物档案和写作计划JSON，等待用户确认。确认后用户可选择**立即开始写作**或**暂停并修改**：选择暂停时，skill 输出人物设定文件（`00-人物档案.json`）与大纲文件（`01-大纲.json`）的完整路径并停止，供用户修改。 → 详见 [phase2-planning.md](references/flows/phase2-planning.md)
...
[L] 修辞手法去重优化 → 运行第1001阶段（限制比喻/拟人/排比/夸张频次，以白描和对话为主的风格为目标，在所有优化之后最后执行）
